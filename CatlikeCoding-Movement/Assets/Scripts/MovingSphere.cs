...
public class MovingSphere : MonoBehaviour
{
	...
	bool SnapToGround() //este metodo lo que hará es que la bola se quede stucked en el suelo 
	{
		...
		if(!Physics.Raycast(body.position, Vector3.down, out RaycastHit hit, probeDistance, probeMask)) //esto comprueba si hay suelo debajo, ya que si la condicion de arriba no se da, y entre dos superficies digamos que hay un vacio y esta pensado para que el jugador lo tenga que saltar, si solo es un frame
		...
		if(hit.normal.y < GetMinDot(hit.collider.gameObject.layer)) //con lo que choca el raycast es retornado a traves de outRaycast, de esto podemos hcer el vector normal y calcular si entra dentro del rango que tenemos considerado que la bola esta en el aire o no.
	...
	void Jump()
    {
        ...
        else
        {
			...
			jumpDirection = (jumpDirection + Vector3.up).normalized; //esto es lo que nos permite escalar muros
